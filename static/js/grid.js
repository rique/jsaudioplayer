/** Grid Module
* Provides a flexible and extensible grid system for displaying tabular data, such as tracklists and queuelists.
* Supports features like sorting, searching, and drag-and-drop reordering of rows and cells.
* Includes a GridMaker class to facilitate the creation and management of grids, allowing for easy configuration of row and cell properties.
* Integrates with the TrackEditor and TrackSearch modules to enable editing and searching of track metadata directly within the grid interface.
* Utilizes event-driven architecture to allow for dynamic updates and interactions with the grid, such as responding to tracklist changes or user actions.
* Designed to be reusable across different components of the application, providing a consistent and efficient way to display and interact with tabular data.
* The module includes the following classes:
* BaseColumn: Represents a column in the grid, managing its cells and providing basic functionality.
* IndexedColumn: Extends BaseColumn to support sorting and indexing of cells based on their content.
* BaseGrid: Represents the grid itself, managing rows and providing methods for rendering, opening, and closing the grid.
* SearchableGrid: Extends BaseGrid to add searching capabilities, allowing for filtering of rows based on search terms.
* SortableGrid: Extends SearchableGrid to add sorting capabilities, allowing for sorting of rows based on column content.
* GridMaker: A utility class for creating and managing grids, providing methods for adding rows, configuring drag-and-drop, and rendering the grid.
* TracklistGrid: A specific implementation of a grid for displaying tracklists, integrating with the audio player and tracklist browser to manage track metadata and interactions.
* The module is designed to be modular and extensible, allowing for easy integration with other components of the application and providing a robust framework for displaying and interacting with tabular data.
*/
...
const TracklistGrid = function(selector, audioPlayer, trackListBrowser) {
